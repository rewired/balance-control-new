...
No measures, no mechanics yet — just the package boundary and integration proof.

## Inputs
- `AGENTS.md` §1.6 (Resource registry), §3.4/§3.8 (Modular expansions), §5.5 (No dead state)
...
- `/packages/exp-01-economy`:
  ...
  - `exp01Economy(): ExpansionModule`:
    - `id: 'exp01'`
    - `registerResources(registry)` registers `ECO`
    - `setupExpansionState(G, ctx)` creates `G.exp.exp01` and minimal exp01-only containers
    - optional stub move: `exp01_noop` that flips a sentinel value in exp01 slice

- `@bc/game`:
  - Import the module in the factory and enable via config.

### Tests
- CORE-only: no ECO key, no exp01 slice, no exp01 moves.
- CORE + exp01: ECO exists, exp01 slice exists, exp01 stub move exists + zod-validated.

### Docs
- `/docs/changelog.md` entry: “Added EXP‑01 package scaffold (stub module).”
